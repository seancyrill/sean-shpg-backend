...
export async function createSinglePromo(req: reqTypes, res: express.Response) {
  try {
    const { shop_id, item_id, discount, start_date, end_date, promo_group_id } = req.body
    ...
    return res.status(200).json({ message: `Successfully added promo to item with ID ${item_id}` })
  } catch (error) {
